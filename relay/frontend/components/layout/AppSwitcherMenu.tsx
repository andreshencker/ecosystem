...
export function AppSwitcherMenu() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get<{ applications: EnabledApp[] }>('/app-switcher').then(r => r.data.applications),
    enabled: open,
  });

  return (
    <>
      <IconButton onClick={event => setAnchorEl(event.currentTarget)} aria-label="Switch apps" sx={{ flexShrink: 0 }}>
        <AppsIcon />
      </IconButton>
      <Popover
        open={open}
