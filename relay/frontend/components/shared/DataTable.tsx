...
export interface DataTableProps<T extends { id?: string }> {
  ...
  /**
   * When provided the DataGrid renders with this fixed height and handles
   * its own vertical scroll (sticky header, paginator always visible).
   * When omitted the default autoHeight behaviour grows with content.
   * Accepts any valid CSS height value: px, %, calc(), max(), etc.
   */
...
export function DataTable<T extends { id?: string }>({
  ...
  const grid = (
    <DataGrid
      ...
      getRowHeight={() => 52}
      // autoHeight only when no fixed tableHeight — fixed height enables internal scroll
      autoHeight={!tableHeight}
      sx={{
        border: 'none',
        // When tableHeight is set the grid must fill its wrapper container
        ...
        // clipped. A minHeight guarantees the overlay has room to render.
  ...
  return (
    <Box>
      {filterNode}
      {tableHeight ? (
        // Fixed-height wrapper: DataGrid handles its own vertical scroll,
        // column headers stay sticky, paginator stays at the bottom.
