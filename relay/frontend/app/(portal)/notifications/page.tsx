  ...
  return (
    <Box sx={{ p: { xs: 3, sm: 4 } }}>
      ...
      <FlowStep n={1} title="Get your integration token" auth="Grapifly session">
        <Body>
          Relay no longer issues this token itself — go to Integrations in Grapifly
          (your organization&apos;s own settings, not Relay) and create a communication
          token there. The raw token is shown <strong>once</strong> — store it securely.
          Every subsequent call from your external app authenticates with this token in
          the <code>x-integration-token</code> header; Relay verifies it live against
          Grapifly and resolves your company from it automatically.
        </Body>
      </FlowStep>
