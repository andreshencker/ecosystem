...
export class EcosystemIdentityService {
  ...
  /**
   * Lighter-weight sibling of resolveGrapiflyCompany() — used for machine-to-machine
   * calls (a communication token presented by an external caller) where there is no
   * specific requesting user, only an organization. Same fast path (find by
   * grapiflyOrganizationId); falls straight to create-if-missing since there's no
   * user context to claim an existing unlinked Company through.
   */
  async resolveGrapiflyCompanyByOrganization(organization: {
    ...
    let companyKey = this.slugify(organization.name);
    if (!companyKey) companyKey = `grapifly-${organization.organizationId.slice(-8).toLowerCase()}`;
    const collision = await this.companyModel.exists({ companyKey });
    if (collision) companyKey = `${companyKey}-${organization.organizationId.slice(-6).toLowerCase()}`;
    return this.companyModel.create({
      grapiflyOrganizationId: organization.organizationId,
      companyKey,
