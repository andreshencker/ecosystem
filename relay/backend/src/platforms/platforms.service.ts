...
export class PlatformsService {
  ...
  async findById(id: string): Promise<PlatformResponseDto> {
    const doc = await this.model.findById(this.toObjectIdOrThrow(id)).lean();
  ...
  async create(dto: CreatePlatformDto): Promise<PlatformResponseDto> {
    ...
    try {
      ...
      if (err?.code === 11000) {
        throw new HttpException('Platform key already exists', HttpStatus.BAD_REQUEST);
      }
  ...
  async update(id: string, dto: UpdatePlatformDto): Promise<PlatformResponseDto> {
    const _id = this.toObjectIdOrThrow(id);
    const $set: any = {};
  ...
  private toObjectIdOrThrow(id: string) {
