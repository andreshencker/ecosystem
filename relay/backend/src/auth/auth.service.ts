...
export class AuthService {
  ...
  /**
   * Relay's own role→permission vocabulary — Grapifly only ever returns the
   * raw applicationRole; each app owns mapping that to its own permissions.
  ...
  private relayPermissions(flow: RelaySessionContext['flow'], role: string): string[] {
    ...
    return role === 'owner'
      ? [...base, ...management, 'relay.members.manage', 'relay.organization.manage']
      : [...base, ...management, 'relay.members.manage'];
  }
