...
export class NotificationController {
   ...
   *
   * Authentication — accepts three methods (guard resolves all before reaching here):
   *   a. x-api-key: <RELAY_API_KEY>  — internal/admin callers
   *   b. x-api-key or x-integration-token: <integration_token>  — external apps (Business App)
   *   c. x-grapifly-service-secret + x-grapifly-organization-id  — Grapifly's own backend
   *
  ...
  async notifyEvent(
    ...
    const isGrapiflyService = authCtx?.keyId === 'grapifly-service';
    // Both the external-app integration token and Grapifly's own service secret
    // were fully resolved+validated by GlobalAuthGuard — companyId on authCtx
    // is already trustworthy, no further assertApiKey() confirmation needed.
    const isIntegration =
      authCtx?.keyId === 'integration-token' || isGrapiflyService;
    const authMethod =
      ctx.actorType === 'user'
        ? 'relay-session'
        : isGrapiflyService
    ...
    // Sanity check for the external integration token only: body companyId should
    // match the resolved companyId. Not applicable to grapifly-service — Grapifly
    // only knows its own organizationId, never Relay's internal Company ObjectId,
    // so body.companyId is expected to differ there (it's overridden regardless).
    if (
