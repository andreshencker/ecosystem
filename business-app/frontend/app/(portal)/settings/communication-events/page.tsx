...
import {
  ...
  useDeleteCommunicationEventMutation,
} from '@/hooks/api/useCommunicationEvents';
import { usePurposes }    from '@/hooks/api/useCommunicationPurposes';
import { useIntegration } from '@/hooks/api/useCommunicationConnection';
import type { CommunicationEvent, CommunicationEventType, ChannelStatus } from '@/types/communication-events';
import { EVENT_TYPE_OPTIONS, getEmailStatus, getSmsStatus } from '@/types/communication-events';
...
const EVENT_TYPE_COLORS: Record<CommunicationEventType, 'primary' | 'warning' | 'success' | 'error'> = {
...
interface EventRowActionsProps {
  ...
  onEdit:   (e: CommunicationEvent) => void;
  onDelete: (e: CommunicationEvent) => void;
}
...
export default function CommunicationEventsPage() {
  ...
  // ── Communication Purposes (domain selector) ──────────────────────────────
  ...
  const [editingEvent, setEditingEvent] = useState<CommunicationEvent | null>(null);

  // ── Confirm delete ────────────────────────────────────────────────────────
  const [confirmDelete, setConfirmDelete] = useState<CommunicationEvent | null>(null);
  ...
  const { data, isLoading, error } = useCommunicationEvents({
  ...
  const deleteMutation = useDeleteCommunicationEventMutation();

  // Client-side search + channel filter on returned data
  const allRows: CommunicationEvent[] = data?.data ?? [];
  const rows = useMemo(() => {
    let result = allRows;
  ...
  function openEdit(e: CommunicationEvent) { setEditingEvent(e); setDrawerOpen(true); }
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      minWidth: 160,
      renderCell: (params: GridRenderCellParams) => {
        const row = params.row as CommunicationEvent;
        return (
    ...
    },
    {
      ...
      width: 115,
      renderCell: (params: GridRenderCellParams) => {
        const type = params.value as CommunicationEventType;
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => (
        <EventRowActions
          row={params.row as CommunicationEvent}
          onEdit={openEdit}
  ...
  const mobileCardConfig: MobileCardConfig<CommunicationEvent> = {
    ...
    fields: [
      {
        ...
        label: 'Type',
        render: (val) => {
          const type = val as CommunicationEventType;
          const label = EVENT_TYPE_OPTIONS.find((o) => o.value === type)?.label ?? type;
  ...
  if (!isConnected) {
    return (
      <Box>
        <PageHeader title="Communication Events" />
        <EmptyState
          icon={CableOutlinedIcon}
          title="Communications not connected"
          description="Connect Business App to Communications before managing Communication Events."
          action={
            <Button
              variant="contained"
    ...
    return (
      <Box>
        ...
        <EmptyState
          ...
          title="No Communication Purposes found"
          description="Create a Communication Purpose before adding Events."
          action={
            <Button
              variant="contained"
              startIcon={<NotificationsNoneOutlinedIcon />}
              onClick={() => router.push('/settings/communication-purposes')}
            >
              Go to Communication Purposes
            </Button>
          }
        />
      </Box>
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Manage notification events within your Communication Purposes."
        actions={
          <Button
            variant="contained"
            startIcon={<AddIcon />}
      ...
      <Box mb={2}>
        <FormControl size="small" sx={{ minWidth: 280 }}>
          <InputLabel>Communication Purpose</InputLabel>
          <Select
      ...
      <ConfirmDialog
        open={!!confirmDelete}
        title="Delete Communication Event?"
        description={
          confirmDelete
