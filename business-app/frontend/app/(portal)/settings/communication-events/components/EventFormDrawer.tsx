...
import {
  useCreateCommunicationEventMutation,
  useUpdateCommunicationEventMutation,
...
import { EVENT_TYPE_OPTIONS } from '@/types/communication-events';
...
function eventToFormValues(event: CommunicationEvent, domainCatalogueId: string): EventFormValues {
...
export function EventFormDrawer({
  ...
  const createMutation = useCreateCommunicationEventMutation();
  const updateMutation = useUpdateCommunicationEventMutation();
  ...
  return (
    <FormDrawer
      open={open}
      onClose={onClose}
      title={isEdit ? `Edit — ${event?.displayName}` : 'New Communication Event'}
      width={560}
      actions={
        <>
        ...
        <Box>
          ...
          <Box display="flex" flexDirection="column" gap={2} mt={1}>
            ...
            <Controller
              ...
              control={control}
              rules={{ required: 'Communication Purpose is required' }}
              render={({ field }) => (
                <TextField
                  {...field}
                  select
