...
import type {
  ShiftInvoiceXmlDto,
  ShiftInvoiceXmlWorkedHoursRowDto,
} from './shift-invoice-xml.dto';

export function mapShiftInvoiceToXml(result: ShiftInvoiceBiResult): ShiftInvoiceXmlDto {
  const workedHours: ShiftInvoiceXmlWorkedHoursRowDto[] = result.workedHours.map((row) => ({
    shiftId:     row.shiftId,
