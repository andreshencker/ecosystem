...
/**
 * Partial update payload for a Communication Event.
 * domainCatalogueId is intentionally excluded — the domain cannot be
 * changed after creation.
 */
export class UpdateCommunicationEventDto {
  ...
  @ApiPropertyOptional({ enum: ['notification', 'alert', 'request', 'security'] })
  @IsOptional()
  @IsEnum(['notification', 'alert', 'request', 'security'])
  eventType?: 'notification' | 'alert' | 'request' | 'security';

  @ApiPropertyOptional({ description: 'Full channel content object (email and/or sms)' })
  @IsOptional()
  @IsObject()
