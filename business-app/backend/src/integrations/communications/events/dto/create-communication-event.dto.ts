...
export class CreateCommunicationEventDto {
  @ApiProperty({ description: 'Communication Purpose (domain) ObjectId', example: '64f...' })
  @IsMongoId()
  domainCatalogueId!: string;

  @ApiProperty({ example: 'invoice_sent', description: 'Unique key — lowercase letters, numbers, hyphens and underscores' })
  @IsString()
  @IsNotEmpty()
  eventKey!: string;
  ...
  @ApiPropertyOptional({ example: 'Triggered when an invoice is sent to a customer' })
  @IsOptional()
  @IsString()
  description?: string;
