 ...
 * All events are delivered via Platform credentials (COMMUNICATION_API_KEY).
 * These are the 7 events that exist in the Communications DB.
 */
  ...
  events: [
    {
      ...
      description: 'Deliver the email verification link to a business user who self-registered. Always delivered via platform credentials.',
      eventType: 'security',
      channels: {
        email: {
          enabled: true,
...
<p style="margin: 0 0 24px; text-align: center;">
  <a href="{{data.loginUrl}}"
    ...
    },
    {
      ...
      description: 'Welcome an invited business user after they complete their first login and set a permanent password.',
      eventType: 'notification',
      channels: {
        email: {
          enabled: true,
