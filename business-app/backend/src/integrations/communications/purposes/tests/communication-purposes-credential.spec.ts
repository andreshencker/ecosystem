 ...
 * — the same shape returned by Communications' findAllByCompany().
 */
function makeProviderCredential(overrides: Partial<Record<string, any>> = {}): Record<string, any> {
  return {
    id:                'cred_abc123',
    tag:               'general',
...
const FAKE_CONN = {
  communicationCompanyId: 'remote_co_001',
...
async function buildService(responseData: any) {
  ...
  return {
    service: module.get(CommunicationPurposesService),
    httpMock,
  };
}
...
describe('CommunicationPurposesService — getCredentialOptions', () => {
  ...
  it('calls /provider-credentials with populate=true and active=true (not /options)', async () => {
    ...
    expect(url).toContain(`companyId=${FAKE_CONN.communicationCompanyId}`);
  });
  ...
  it('maps SMS credential with phone number as displayIdentifier', async () => {
    const raw = makeProviderCredential({
      ...
      companyChannelProvider: {
        id: 'ccp_002', companyId: 'remote_co_001', isActive: true,
        provider: { providerKey: 'twilio', displayName: 'Twilio', connectionType: 'api_key' },
  ...
  it('returns only email credentials when channel=email', async () => {
    const emailCred   = makeProviderCredential({ id: 'email_01', tag: 'general' });
    const smsCred     = makeProviderCredential({
      id: 'sms_01', tag: 'sms_general',
      companyChannelProvider: { ...emailCred.companyChannelProvider,
        channel: { channelKey: 'sms', displayName: 'SMS' } },
    });
  ...
  it('returns only SMS credentials when channel=sms', async () => {
    const emailCred = makeProviderCredential({ id: 'email_01', tag: 'email_tag' });
    const smsCred   = makeProviderCredential({
      id: 'sms_01', tag: 'sms_tag',
      companyChannelProvider: { ...emailCred.companyChannelProvider,
        channel: { channelKey: 'sms', displayName: 'SMS' } },
    });
    const { service } = await buildService(pageOf(emailCred, smsCred));
  ...
  it('excludes calendar credentials from both email and SMS selectors', async () => {
    const calendarCred = makeProviderCredential({
      id: 'cal_01', tag: 'calendar_tag',
      companyChannelProvider: {
        id: 'ccp_cal', companyId: 'remote_co_001', isActive: true,
        provider: { providerKey: 'google', displayName: 'Google', connectionType: 'oauth' },
  ...
  it('excludes inactive credentials (active=true is sent to Communications)', async () => {
    // Communications filters inactive records server-side when active=true is in the query.
  ...
  it('falls back to providerKey when providerDisplayName is missing', async () => {
    const raw = makeProviderCredential({
      companyChannelProvider: {
        ...
        provider: { providerKey: 'sendgrid', displayName: '', connectionType: 'api_key' },
      },
    });
    const { service } = await buildService(pageOf(raw));
  ...
  it('returns empty connectionType string when field is absent', async () => {
    const raw = makeProviderCredential({
      companyChannelProvider: {
        ...makeProviderCredential().companyChannelProvider,
        provider: { providerKey: 'smtp', displayName: 'SMTP', connectionType: undefined },
      },
    });
    const { service } = await buildService(pageOf(raw));
  ...
  it('does not crash when companyChannelProvider is missing nested fields', async () => {
    const raw = makeProviderCredential({
      companyChannelProvider: {
        id: 'ccp_broken', companyId: 'remote_co_001', isActive: true,
        provider: undefined,
  ...
  it('resolves id from _id when id field is absent', async () => {
    const raw = { ...makeProviderCredential(), id: undefined, _id: 'mongo_id_001' };
    const { service } = await buildService(pageOf(raw));
  ...
  it('returns multiple email credentials when all match the channel', async () => {
    ...
    const cred2 = makeProviderCredential({ id: 'cred_2', tag: 'marketing',
      displayIdentifier: 'marketing@company.com',
      companyChannelProvider: {
        ...cred1.companyChannelProvider,
        provider: { providerKey: 'sendgrid', displayName: 'SendGrid', connectionType: 'api_key' },
      },
    });
  ...
  it('returns empty array when Communications returns an error', async () => {
    const error = Object.assign(new Error('network error'), { response: { status: 502 } });
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommunicationPurposesService,
        { provide: CommunicationConnectionService, useValue: mockConnectionService },
        {
          provide: HttpService,
          useValue: { get: jest.fn().mockImplementation(() => { throw error; }) },
        },
        { provide: ConfigService, useValue: mockConfig },
      ],
    }).compile();
    const svc = module.get(CommunicationPurposesService);
