...
/**
 * Shifts domain — platform events for shift (turno) lifecycle actions.
 * All events delivered via Platform credentials (RELAY_API_KEY).
 * These are internal ERP notifications — NOT relay to customers.
 *
 * Adding new events here is sufficient for them to be provisioned automatically
 * in Relay on next application startup (via provisionPlatformCatalog).
 */
export const SHIFTS_SEED_DOMAIN: CatalogDomain = {
  domainKey: 'shifts',
  displayName: 'Shifts',
