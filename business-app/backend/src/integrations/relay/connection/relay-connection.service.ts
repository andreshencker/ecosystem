...
import { IntegrationConnectionResponseDto } from './dto/relay-connection.dto';
 ...
 *
 * relayCompanyId = remoteCompanyId: the ID of this Business in Relay App.
 * decryptedToken         = plaintext integration token (never log in full).
 */
export interface BusinessConnection {
  relayCompanyId: string;
...
export class RelayConnectionService {
  private readonly logger = new Logger(RelayConnectionService.name);
   ...
   * Returns the Relay companyId as remoteCompanyId.
   */
  private async verifyTokenWithRemote(token: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        remoteCompanyId: d.companyId, // Relay App's own field name
      ...
      const message = e?.response?.status
        ...
        : e?.code === 'ECONNREFUSED' || e?.code === 'ENOTFOUND'
          ? 'Cannot reach the Relay server — verify RELAY_API_URL'
  ...
  async getRelayConnectionForContext(
    ...
    if (type === 'platform') {
      ...
      if (!platformBusiness?._id) {
        this.logger.warn(
          '[getRelayConnectionForContext] type=platform: no Business with isPlatformCompany=true found — returning null.',
        );
        return null;
      }
    ...
    if (!businessId) {
      this.logger.error(
        '[getRelayConnectionForContext] type=business requires businessId — none provided.',
      );
      return null;
    }
  ...
  async getConnectionForBusiness(
    ...
    const doc = (await this.model
      .findOne({
        ...
        // Persisted value stays 'communications' — see relay-connection.controller.ts PROVIDER comment.
    ...
    // remoteCompanyId may be null if Relay was unreachable during save.
    // The decrypted token is still usable for outbound API calls (Relay
    ...
    try {
      ...
      return {
        relayCompanyId: doc.remoteCompanyId ?? '',
   ...
   * Verifies the token against Relay to resolve remoteCompanyId.
   * If Relay is unreachable, saves without remoteCompanyId (notifications will be skipped).
   *
   * WHY two-step upsert:
  ...
  /** Find all connections with an active Relay connection (for catalog sync). */
