...
async function buildService(http: {
  ...
  const httpMock = {
    ...
    delete: jest
      .fn()
      .mockReturnValue(of({ data: { deleted: true }, status: 200 })),
    ...http,
  };
  ...
  return { service: module.get(RelayEventsService), httpMock };
}

// ─── Suite ────────────────────────────────────────────────────────────────────

describe('RelayEventsService', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('token resolution', () => {
    it('resolves the business connection on every request', async () => {
      ...
      expect(
        mockConnectionService.getRelayConnectionForContext,
  ...
  describe('list', () => {
    it('calls /event-catalogue with domainCatalogueId and pagination', async () => {
      const { service, httpMock } = await buildService({
        get: jest
          .fn()
          .mockReturnValue(of({ data: pageOf(makeEvent()), status: 200 })),
      });
    ...
    it('passes active filter to Relay when set', async () => {
      const { service, httpMock } = await buildService({});
      await service.list('biz_001', {
        domainCatalogueId: DOMAIN_ID,
        active: false,
  ...
  describe('findOne', () => {
    ...
    it('throws 403 when domain belongs to a different company', async () => {
      ...
      await expect(service.findOne('biz_001', EVENT_ID)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
    });
  });
  ...
  describe('create', () => {
    ...
    it('throws 403 when domain belongs to a different company', async () => {
      const { service } = await buildService({
        get: jest.fn().mockReturnValue(
          of({
            data: { ...FAKE_DOMAIN, companyId: 'other_co' },
            status: 200,
          }),
        ),
      });
    ...
    it('verifies ownership then PATCHes the event', async () => {
      const { service, httpMock } = await buildService({
        ...
        patch: jest
          .fn()
          .mockReturnValue(
            of({ data: makeEvent({ displayName: 'Updated' }), status: 200 }),
          ),
      });

      const result = await service.update('biz_001', EVENT_ID, {
        displayName: 'Updated',
      });
  ...
  describe('remove', () => {
    ...
    it('throws 403 when event belongs to a different company', async () => {
      ...
      await expect(service.remove('biz_001', EVENT_ID)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
    });
  });
    ...
    it('verifies domain ownership then POSTs to /event-catalogue/bulk', async () => {
      const { service, httpMock } = await buildService({
        ...
        post: jest
          .fn()
          .mockReturnValue(
            of({ data: [makeEvent(), makeEvent()], status: 201 }),
          ),
      });
  ...
  describe('error mapping', () => {
    it('forwards Relay 400 message to the caller', async () => {
  ...
  describe('no local persistence', () => {
    it('does not inject a Mongoose model', () => {
      ...
      const meta = Reflect.getMetadata('design:paramtypes', RelayEventsService);
