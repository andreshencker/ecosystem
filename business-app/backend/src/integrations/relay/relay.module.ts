...
} from './connection/relay-connection.schema';
import { RelayConnectionService } from './connection/relay-connection.service';
import { RelayConnectionController } from './connection/relay-connection.controller';
...
/**
 * RelayModule — Integration with Relay App.
 *
 * On startup, provisions Platform Events to Relay App idempotently.
...
export class RelayModule implements OnApplicationBootstrap {
  private readonly logger = new Logger(RelayModule.name);
  ...
  onApplicationBootstrap(): void {
    this.dropStaleCompanyProviderIndex()
      ...
      .catch((err: unknown) => {
        ...
        this.logger.error(
          '[onApplicationBootstrap] Startup sequence failed unexpectedly: ' +
            msg,
        );
      });
  }
  ...
  private async dropStaleCompanyProviderIndex(): Promise<void> {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      this.logger.warn(
        `[startup] Could not drop stale index (non-fatal): ${msg}`,
      );
    }
  }
}
