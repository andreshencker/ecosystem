import { validateCatalog, assertCatalogValid } from './relay-catalog.validator';
...
describe('assertCatalogValid', () => {
  ...
  it('throws for an invalid catalog', () => {
    ...
    expect(() => assertCatalogValid(bad)).toThrow('RELAY_CATALOG is invalid');
  });
});

describe('RELAY_CATALOG', () => {
  it('is itself valid', () => {
    expect(validateCatalog(RELAY_CATALOG)).toHaveLength(0);
  });

  it('has platform section with at least one domain', () => {
    expect(RELAY_CATALOG.platform.length).toBeGreaterThan(0);
  });

  it('contains the security domain in platform', () => {
    const security = RELAY_CATALOG.platform.find(
