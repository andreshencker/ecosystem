...
import { findCatalogEvent } from '../catalog/relay-catalog';
import type {
  CatalogDomain,
  CatalogEvent,
...
/**
 * RelayClientService — single point of contact for all HTTP calls to Relay App.
 *
 * Rules:
 *   - No other service makes direct HTTP calls to Relay App.
 *   - type='platform' uses the base company connection; type='business' uses the Business's own.
 *   - The routing decision (type) is internal — Relay never sees it.
 *   - Relay always receives: { companyId: remoteCompanyId, event, email, payload } + x-api-key.
...
export class RelayClientService {
  ...
  /**
   * Send a notification event to Relay App.
  ...
  async notifyEvent(params: NotifyEventParams): Promise<boolean> {
    ...
    if (!catalogEntry) {
      this.logger.error(
        `${tag} REJECTED — event "${params.event}" not found in RELAY_CATALOG. ` +
    ...
    if (!conn) {
      if (params.type === 'platform') {
        this.logger.error(
          `${tag} SKIPPED — platform base company has no active RelayConnection. ` +
            'Configure the platform company connection in Settings → Relay.',
        );
      } else {
        this.logger.warn(
          `${tag} SKIPPED — no active RelayConnection for businessId=${params.businessId}. ` +
            'Business must configure its Relay integration first.',
        );
      }
      return false;
    }
    ...
    const body = {
      companyId: conn.relayCompanyId, // remoteCompanyId — Relay App's own field
      event: params.event,
      email: params.email,
    ...
    this.logger.log(
      `${tag} POST ${url} remoteCompanyId=${conn.relayCompanyId} token=${conn.decryptedToken.slice(0, 12)}...`,
    );
    ...
    try {
      ...
      const responseMsg =
        typeof e?.response?.data === 'object' && e.response.data !== null
          ? ((e.response.data as Record<string, unknown>)['message'] as
              string | undefined)
          : undefined;
      this.logger.error(
  ...
  /**
   * Verify a raw integration token against Relay App.
   * Returns remoteCompanyId (Relay App's companyId for this Business).
   */
  async verifyIntegrationToken(rawToken: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        remoteCompanyId: d.companyId, // Relay App uses "companyId" internally
   ...
   * apiKey = RELAY_API_KEY (for platform) or integration token (for business).
   *
   ...
   * (Relay App uses 'company' internally for business events).
   */
  async createEvent(
    ...
    // Build channelContent in the format Relay App expects.
  ...
  async generateDocument(params: {
    ...
    if (!conn?.relayCompanyId) {
      throw new Error('Active verified Relay connection is required');
    }
