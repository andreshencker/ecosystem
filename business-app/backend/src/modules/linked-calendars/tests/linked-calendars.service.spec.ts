...
const REMOTE_CALS = [
  ...
  {
    externalCalendarId: CAL_LASSO,
    calendarName: 'LASSO Calendar',
    calendarDescription: 'External roster',
    timezone: 'Australia/Sydney',
...
describe('LinkedCalendarsService', () => {
  ...
  let calendarClient: jest.Mocked<RelayCalendarClient>;
  let commClient: jest.Mocked<RelayClientService>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: RelayCalendarClient, useValue: calendarClient },
  ...
  it('T1: throws ServiceUnavailableException when Relay is not configured', async () => {
    calendarClient.listCalendarAccounts.mockRejectedValueOnce(
      new ServiceUnavailableException('Relay integration is not configured'),
    );
    await expect(service.listAvailableAccounts(COMPANY_ID)).rejects.toThrow(
      ServiceUnavailableException,
    );
  });
  ...
  it('T2: returns available calendar accounts from Relay', async () => {
    const result = await service.listAvailableAccounts(COMPANY_ID);
    expect(result).toHaveLength(1);
    expect(result[0].connectionId).toBe(CONN_ID);
    expect(calendarClient.listCalendarAccounts).toHaveBeenCalledWith(
      COMPANY_ID,
    );
  });
  ...
  it('T3: delegates calendar-channel filtering to the Relay client', async () => {
  ...
  it('T5: excludes already-linked calendars from the available list', async () => {
    ...
    expect(
      result.find((c) => c.externalCalendarId === CAL_WORK),
    ).toBeUndefined();
    // CAL_LASSO is not linked — must still appear
    expect(
      result.find((c) => c.externalCalendarId === CAL_LASSO),
    ).toBeDefined();
    expect(result).toHaveLength(1);
  });
  ...
  it('T6: links a single calendar and sends calendar_linked notification', async () => {
    ...
    const result = await service.linkCalendars(
      COMPANY_ID,
      { connectionId: CONN_ID, calendarIds: [CAL_WORK] },
      ACTOR,
    );
    await flushPromises();
    ...
    expect(commClient.notifyEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'business',
        event: 'linked_calendars.calendar_linked',
      }),
    );
  });
  ...
  it('T7: bulk links multiple calendars and sends bulk notification', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain(docs[callIdx++ % 2]));
    model.findOne = jest
      .fn()
      .mockImplementation(() =>
        chain({ status: 'active', createdAt: NOW, updatedAt: NOW }),
    ...
    await service.linkCalendars(
      COMPANY_ID,
      { connectionId: CONN_ID, calendarIds: [CAL_WORK, CAL_LASSO] },
      ACTOR,
    );
    await flushPromises();
    ...
    expect(commClient.notifyEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'business',
        event: 'linked_calendars.calendars_bulk_linked',
      }),
    );
  });
  ...
  it('T8: upsert (findOneAndUpdate) called once per calendar per request', async () => {
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain(makeDoc()));
    model.findOne = jest
      .fn()
      .mockImplementation(() =>
        chain({ status: 'active', createdAt: NOW, updatedAt: NOW }),
  ...
  it('T9: reactivates a paused calendar and sends calendar_activated notification', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain(pausedDoc));
  ...
  it('T10: rejects calendarIds not returned by Relay', async () => {
    await expect(
      service.linkCalendars(
        COMPANY_ID,
        { connectionId: CONN_ID, calendarIds: ['non-existent-id'] },
        ACTOR,
      ),
  ...
  it('T12: findOrThrow returns NotFoundException for another tenant', async () => {
    ...
    await expect(service.findById('linked-001', OTHER_CO)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('T13: pauses an active calendar and sends notification', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain({ ...activeDoc, status: 'paused' }));
  ...
  it('T14: activates a paused calendar and sends notification', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain({ ...pausedDoc, status: 'active' }));
  ...
  it('T17: all write notifications use type: business (business-owned operational events)', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain({ ...makeDoc(), status: 'paused' }));
  ...
  it('T18: notification failure does not roll back the operation', async () => {
    ...
    await expect(
      service.unlink('linked-001', COMPANY_ID, ACTOR),
  ...
  it('T19: linkCalendars stores no provider tokens or credentials', async () => {
    ...
    const [$set] = model.findOneAndUpdate.mock.calls.map((c) => c[1].$set);
    const forbidden = [
      'token',
      'accessToken',
      'refreshToken',
  ...
  it('T21: linkCalendars stores flow from DTO when provided', async () => {
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain(makeDoc({ flow: 'holidays' })));
    model.findOne = jest
      .fn()
      .mockImplementation(() =>
        chain({ status: 'active', createdAt: NOW, updatedAt: NOW }),
    ...
    await service.linkCalendars(
      COMPANY_ID,
      { connectionId: CONN_ID, calendarIds: [CAL_WORK], flow: 'holidays' },
      ACTOR,
    );
  ...
  it('T23: updateStatus updates flow field and does not send notification for flow-only change', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain({ ...doc, flow: 'holidays' }));
  ...
  it('T24: updateStatus updates both status and flow simultaneously', async () => {
    ...
    model.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() =>
        chain({ ...doc, status: 'active', flow: 'payments' }),
      );

    const result = await service.updateStatus(
      'linked-001',
      COMPANY_ID,
      { status: 'active', flow: 'payments' },
      ACTOR,
    );
  ...
  it('T28b: mapper includes flow field when assigned', () => {
    const dto = LinkedCalendarMapper.toResponse(
      makeDoc({ flow: 'payments' }) as any,
    );
  ...
  it('T31: provider calendar with an unrelated ID and name is shown as available', async () => {
    ...
    model.find = jest.fn().mockImplementation(() =>
      chain([
        {
          externalCalendarId: 'different-id',
          calendarName: 'Something Else',
          status: 'active',
          _id: 'local-999',
        },
      ]),
  ...
  it('T32: excludes calendar by name fallback when stored externalCalendarId is a legacy wrong value', async () => {
    ...
    model.find = jest.fn().mockImplementation(() =>
      chain([
        {
          externalCalendarId: 'https://caldav.icloud.com/12345',
          calendarName: 'Work',
          status: 'active',
          _id: 'local-002',
        },
  ...
  it('T35: unlink throws BadRequestException when calendar is referenced as holiday calendar in a Contract', async () => {
    model.findOne = jest.fn().mockImplementation(() => chain(makeDoc()));
    contractModel.findOne = jest
      .fn()
      .mockImplementation(() => chain({ _id: 'contract-001' }));
  ...
  it('T36: unlink throws BadRequestException when calendar is referenced as payment calendar in a Contract', async () => {
    ...
    contractModel.findOne = jest
      .fn()
      .mockImplementation(() => chain({ _id: 'contract-002' }));
  ...
  it('T37: contract reference check uses the tenant businessId for isolation', async () => {
    ...
    const [filter] = contractModel.findOne.mock.calls[0];
  ...
  it('T39: unlink removes only the local linked-calendar record, never calls calendarClient delete', async () => {
    ...
    expect(
      clientKeys.filter((k) => k.toLowerCase().includes('delete')),
    ).toHaveLength(0);
...
async function buildSubscribeModule(
  calendarClientOverrides: Record<string, any> = {},
) {
  const calClient = {
    listCalendarAccounts: jest.fn().mockResolvedValue(ACCOUNTS),
    listCalendars: jest.fn().mockResolvedValue(REMOTE_CALS),
  ...
  const module = await Test.createTestingModule({
    providers: [
      ...
      { provide: RelayCalendarClient, useValue: calClient },
      { provide: RelayClientService, useValue: commClient },
      { provide: UsersService, useValue: usersService },
    ],
  }).compile();
...
describe('LinkedCalendarsService.subscribeByUrl', () => {
  ...
  it('T31: creates and links a calendar from a valid HTTPS URL with correct flow', async () => {
    ...
    const $set = mockModel.findOneAndUpdate.mock.calls[0][1].$set;
    expect($set.flow).toBe('holidays');
  });
  ...
  it('T32: forwards optional calendarName and description to the client', async () => {
    const { service, calClient } = await buildSubscribeModule({
      subscribeToUrl: jest.fn().mockResolvedValue({
        externalCalendarId: 'sub-001',
        calendarName: 'Overridden Name',
        calendarDescription: null,
        timezone: null,
        accessRole: 'read-only',
        isPrimary: false,
  ...
  it('T36: duplicate subscription returns updated record without creating a new one', async () => {
    ...
    mockModel.findOneAndUpdate = jest
      .fn()
      .mockImplementation(() => chain({ ...existingDoc, flow: 'shifts' }));

    const result = await service.subscribeByUrl(
      COMPANY_ID,
      {
        connectionId: CONN_ID,
        subscriptionUrl: VALID_URL,
...
describe('LinkedCalendarsService.subscribeFromCatalogue', () => {
  ...
  it('T38: subscribes to an available catalogue entry with correct flow', async () => {
    ...
    const { service, calClient, mockModel } = await buildSubscribeModule({
      subscribeToUrl: jest.fn().mockResolvedValue({
        externalCalendarId: 'au-national-001',
    ...
    await service.subscribeFromCatalogue(
      COMPANY_ID,
      {
        connectionId: CONN_ID,
        catalogueKey: AU_NATIONAL_KEY,
        flow: 'holidays',
      },
      ACTOR,
    );
  ...
  it('T39: rejects unknown catalogue key', async () => {
    ...
    await expect(
      service.subscribeFromCatalogue(
        COMPANY_ID,
        {
          connectionId: CONN_ID,
          catalogueKey: 'non_existent_key',
          flow: 'holidays',
        },
        ACTOR,
      ),
...
describe('LinkedCalendarsService.setupPaymentCalendar', () => {
  ...
  it('T50: creates a payment calendar when none exists', async () => {
    ...
    const result = await service.setupPaymentCalendar(
      COMPANY_ID,
      { connectionId: CONN_ID },
      ACTOR,
    );
  ...
  it('T51: assigns payments flow to the created calendar', async () => {
    ...
    await service.setupPaymentCalendar(
      COMPANY_ID,
      { connectionId: CONN_ID },
      ACTOR,
    );
  ...
  it('T56: tenant isolation — only checks calendars for this companyId', async () => {
    ...
    const filterArg = mockModel.findOne.mock.calls[0][0];
...
describe('LinkedCalendarsService.setupAustralianHolidays', () => {
  ...
  it('T57: subscribes to AU national holidays when none exists', async () => {
    ...
    await service.setupAustralianHolidays(
      COMPANY_ID,
      { connectionId: CONN_ID },
      ACTOR,
    );
  ...
  it('T59: returns wasExisting=false when a new subscription is created', async () => {
    ...
    const result = await service.setupAustralianHolidays(
      COMPANY_ID,
      { connectionId: CONN_ID },
      ACTOR,
    );
  ...
  it('T61: propagates ServiceUnavailableException when Relay is unreachable', async () => {
    const { service } = await buildSubscribeModule({
      subscribeToUrl: jest
        .fn()
      ...
      subscribeToUrl: jest
        ...
        .mockRejectedValue(
          new ServiceUnavailableException(
            'Internal provider credentials expired: token xyz123',
          ),
        ),
    });
  ...
  it('T63: rejects when account does not belong to the Business', async () => {
    ...
    await expect(
      service.setupAustralianHolidays(
        OTHER_CO,
        { connectionId: CONN_ID },
        { ...ACTOR, companyId: OTHER_CO },
      ),
...
describe('LinkedCalendarsService.discoverAustralianHolidays', () => {
  ...
  it('T68: returns linked when active holidays calendar already exists (idempotency)', async () => {
    ...
    const result = await service.discoverAustralianHolidays(
      COMPANY_ID,
      { connectionId: CONN_ID },
      ACTOR,
    );
  ...
  it('T69: throws BadRequestException when account not found', async () => {
    ...
    await expect(
      service.discoverAustralianHolidays(
        COMPANY_ID,
        { connectionId: CONN_ID },
        ACTOR,
      ),
  ...
  it('T72: auto-links single candidate with flow=holidays', async () => {
    ...
    await service.discoverAustralianHolidays(
      COMPANY_ID,
      { connectionId: CONN_ID },
      ACTOR,
    );
  ...
  it('T73: returns multiple_matches when multiple candidates found', async () => {
    const { service } = await buildSubscribeModule({
      listCalendars: jest
        .fn()
        .mockResolvedValue([REMOTE_AU_HOLIDAY, REMOTE_AU_HOLIDAY_2]),
    });
    ...
    if (result.status === 'multiple_matches') {
      ...
      expect(result.options[0].externalCalendarId).toBe(
        REMOTE_AU_HOLIDAY.externalCalendarId,
      );
    }
  });
...
describe('LinkedCalendarsService.linkProviderCalendarAsHoliday', () => {
  ...
  it('T75: returns linked (idempotency) when active holidays calendar already exists', async () => {
    ...
    const result = await service.linkProviderCalendarAsHoliday(
      ...
      {
        connectionId: CONN_ID,
        externalCalendarId: PROVIDER_CAL.externalCalendarId,
      },
      ACTOR,
    );
