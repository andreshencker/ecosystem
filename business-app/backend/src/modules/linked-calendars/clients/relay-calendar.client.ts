...
import { RelayConnectionService } from '../../../integrations/relay/connection/relay-connection.service';
...
/** Shape returned by GET /calendar/connections (Relay App). */
...
/** Shape of each item in GET /calendar/connections/:credId/calendars (Relay App). */
 ...
 * RelayCalendarClient
 *
 * Focused HTTP client for Business App → Relay App calendar API calls.
 *
 * Rules:
 * - Reuses existing RelayConnectionService for token resolution.
 * - Sends the integration token as x-api-key (Relay resolves companyId from it).
...
export class RelayCalendarClient {
  private readonly logger = new Logger(RelayCalendarClient.name);
  ...
  /**
   * Resolves the Business's Relay integration token.
  ...
  async resolveConnection(businessId: string): Promise<CalendarCallContext> {
    ...
    if (!conn) {
      throw new ServiceUnavailableException(
        'Relay integration is not configured for this Business. ' +
          'Go to Settings → Relay to connect your integration token.',
      );
    }
  ...
  /**
   * List active calendar accounts (connections) from Relay App.
  ...
  async listCalendarAccounts(
    ...
    try {
      ...
      throw new ServiceUnavailableException(
        `Could not retrieve calendar accounts from Relay: ${err?.message ?? 'Unknown error'}`,
      );
    }
  }

  /**
   * List calendars for a specific connection from Relay App.
  ...
  > {
    ...
    try {
      ...
      throw new ServiceUnavailableException(
        `Could not retrieve calendars from Relay: ${err?.message ?? 'Unknown error'}`,
      );
    }
  }
  ...
  /**
   * Create a new calendar in the provider via Relay App.
  ...
  async createCalendar(
    ...
    try {
      const res = await firstValueFrom(
        this.http.post<{ data?: CommCalendarInfo; id?: string; name?: string }>(
          url,
          body,
          {
            headers: { 'x-api-key': ctx.decryptedToken },
      ...
      // Relay may return { data: CalendarInfo } or the object directly
      const raw: CommCalendarInfo =
  ...
  /**
   * Subscribe a provider account to an external iCal/ICS URL via Relay App.
   ...
   * may not. If unsupported, Relay returns a 422 which maps to a
  ...
  /**
   * Fetch events for a specific calendar from Relay App.
  ...
  async listCalendarEvents(
    ...
    try {
      const res = await firstValueFrom(
        this.http.get<{
          data: { items: CommCalendarEventInfo[] };
          items?: CommCalendarEventInfo[];
        }>(url, {
          headers: { 'x-api-key': ctx.decryptedToken },
          params: {
      ...
      // Relay returns either { data: { items: [] } } or { items: [] }
      const raw = res.data;
      ...
      throw new ServiceUnavailableException(
        `Could not retrieve events from Relay (HTTP ${status ?? 'err'}): ${err?.message ?? 'Unknown error'}`,
      );
    }
  }

  /**
   * Create a new event in an external calendar through Relay App.
  ...
  async createCalendarEvent(
    ...
    try {
      const res = await firstValueFrom(
        this.http.post<
          { data?: CommCalendarEventInfo } | CommCalendarEventInfo
        >(url, event, {
          headers: { 'x-api-key': ctx.decryptedToken },
          timeout: 15_000,
        }),
      ...
      throw new ServiceUnavailableException(
        `Could not create calendar event via Relay (HTTP ${status ?? 'err'}): ${err?.message ?? 'Unknown error'}`,
      );
    }
  }

  /**
   * Update an existing event in an external calendar through Relay App.
  ...
  async updateCalendarEvent(
    ...
    try {
      ...
      throw new ServiceUnavailableException(
        `Could not update calendar event via Relay (HTTP ${status ?? 'err'}): ${err?.message ?? 'Unknown error'}`,
      );
    }
  }

  /**
   * Delete an event from an external calendar through Relay App.
  ...
  async deleteCalendarEvent(
    ...
    try {
      ...
      if (status === 404) {
        ...
        this.logger.log(
          `[deleteCalendarEvent] eventId=${eventId.slice(0, 40)} already absent in provider (404) — treating as success`,
        );
        return true;
      }
      this.logger.error(
      ...
      throw new ServiceUnavailableException(
        `Could not delete calendar event via Relay (HTTP ${status ?? 'err'}): ${err?.message ?? 'Unknown error'}`,
      );
    }
  }
}
...
/** Normalized calendar event as returned by Relay. */
