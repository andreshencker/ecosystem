...
import { Contract, ContractDocument, VALID_RATE_DAYS } from './schemas/contract.schema';
import { Customer, CustomerDocument } from '../customer/schemas/customer.schema';
import { Shift } from '../shifts/schemas/shift.schema';
import { LinkedCalendar, LinkedCalendarDocument } from '../linked-calendars/schemas/linked-calendar.schema';
...
export class ContractsService {
  ...
  private async assertCalendarFlow(
    ...
    if (!cal) {
      throw new NotFoundException(`${expectedFlow} calendar not found or does not belong to this business`);
    }
  ...
  private validateRates(
    rateType: string,
    rates: { days: string[]; startTime?: string | null; endTime?: string | null; hourlyRate: number }[],
  ): void {
    ...
    for (const rule of rates) {
      if (!rule.days || rule.days.length === 0) {
        throw new BadRequestException('Each rate rule must specify at least one day');
      }
    ...
    if (rateType === 'fixed') {
      if (rates.length !== 1) {
        throw new BadRequestException('Fixed rateType must have exactly one rate rule');
      }
      const rule = rates[0];
      if (rule.days.length !== 1 || rule.days[0] !== 'all') {
        throw new BadRequestException('Fixed rate rule must have days: ["all"]');
      }
  ...
  async create(
    ...
    if (dto.paymentCalendarEnabled) {
      if (!dto.paymentCalendarSubscriptionId) {
        throw new BadRequestException('paymentCalendarSubscriptionId is required when paymentCalendarEnabled is true');
      }
    ...
    const doc = await this.model.create({
      ...
      scheduledPaymentDay: ((dto.scheduledPaymentEnabled ?? false) ? (dto.scheduledPaymentDay ?? null) : null) as any,
  ...
  async findAll(
    ...
    const customerIds = [...new Set((items as any[]).map((c) => String(c.customerId)).filter(Boolean))];
    const customers   = customerIds.length > 0
      ? await this.customerModel
          ...
          .exec()
      : [];
    const customerNameMap = new Map((customers as any[]).map((c) => [String(c._id), c.displayName ?? null]));
  ...
  async findById(id: string, businessId: string): Promise<ContractDocument | null> {
    if (!Types.ObjectId.isValid(id)) return null;
    return this.model.findOne({ _id: id, businessId }).lean().exec();
  }

  async findByIdOrThrow(id: string, businessId: string): Promise<ContractDocument> {
    const doc = await this.findById(id, businessId);
    if (!doc) throw new NotFoundException('Contract not found');
    return doc;
  }

  async findByCustomer(customerId: string, businessId: string): Promise<ContractDocument[]> {
    return this.model
  ...
  async update(
    ...
    if ((existing as any).status === 'finished' || (existing as any).status === 'cancelled') {
      throw new BadRequestException(
    ...
    if (dto.endDate || dto.startDate) {
      const startDate = dto.startDate ?? (existing as any).startDate?.toISOString();
    ...
    if (dto.paymentCalendarEnabled) {
      ...
      const existingPayCalId = (existing as any).paymentCalendarSubscriptionId ?? null;
    ...
    if (dto.positionName !== undefined)       $set.positionName = dto.positionName.trim();
    ...
    if (dto.scheduledPaymentEnabled !== undefined) {
      ...
      if (dto.paymentTermsDays !== undefined)  $set.paymentTermsDays   = dto.paymentTermsDays;
      if (dto.scheduledPaymentDay !== undefined) $set.scheduledPaymentDay = dto.scheduledPaymentDay;
    }
    if (dto.rateType !== undefined)           $set.rateType = dto.rateType;
    if (dto.minimumHours !== undefined)       $set.minimumHours = dto.minimumHours;
    if (dto.defaultBreakMinutes !== undefined) $set.defaultBreakMinutes = dto.defaultBreakMinutes;
    if (dto.rates !== undefined)              $set.rates = dto.rates;
    if (dto.notes !== undefined)              $set.notes = dto.notes?.trim() ?? null;
    if (dto.useInvoicePrefix !== undefined)      $set.useInvoicePrefix = dto.useInvoicePrefix;
    if (dto.invoicePrefix !== undefined)         $set.invoicePrefix = dto.invoicePrefix?.trim() ?? null;
    if (dto.startingInvoiceNumber !== undefined) $set.startingInvoiceNumber = dto.startingInvoiceNumber;
    if (dto.currency !== undefined)              $set.currency = dto.currency;
    if (dto.chargeGst !== undefined)             $set.chargeGst = dto.chargeGst;
    if (dto.gstRate !== undefined)               $set.gstRate = dto.chargeGst ? (dto.gstRate ?? null) : null;
  ...
  async activate(id: string, businessId: string, actor: ActorContext): Promise<ContractDocument> {
  ...
  async cancel(id: string, businessId: string, actor: ActorContext): Promise<ContractDocument> {
    const doc = await this.findByIdOrThrow(id, businessId);
    const current = (doc as any).status as string;
  ...
  async finish(id: string, businessId: string, actor: ActorContext): Promise<ContractDocument> {
    const doc = await this.findByIdOrThrow(id, businessId);
    const current = (doc as any).status as string;
