...
describe('ShiftsService — Business App → Calendar (bidirectional)', () => {
  ...
  beforeEach(async () => {
    ...
    mockCustomerModel.findById.mockReturnValue(
      leanExec({ _id: 'cust1', displayName: 'Test Customer' }),
    );
  ...
  it('creates a local-only shift when no linkedCalendarId is supplied', async () => {
    mockShiftModel.create.mockResolvedValue(
      makeShift({ linkedCalendarId: null, syncStatus: null }),
    );
  ...
  it('creates local Shift and pushes event to provider calendar when linkedCalendarId given', async () => {
    const pendingShift = makeShift({
      syncStatus: 'pending',
      externalOccurrenceId: null,
    });
    const syncedShift = makeShift({
      syncStatus: 'synced',
      externalOccurrenceId: OCC_ID,
    });

    mockShiftModel.create.mockResolvedValue(pendingShift);
    mockCalendarClient.createCalendarEvent.mockResolvedValue(
      makeCreatedEvent(),
    );
  ...
  it('uses contract positionName as default title when title omitted', async () => {
    ...
    const calEventCall =
      mockCalendarClient.createCalendarEvent.mock.calls[0][3];
  ...
  it('external update failure does not undo local update', async () => {
    ...
    mockShiftModel.findOne.mockReturnValue(leanExec(linked));
    mockShiftModel.findOneAndUpdate.mockReturnValue(
      leanExec({ ...linked, location: 'New Office' }),
    );
    mockContractModel.findById.mockReturnValue(leanExec(makeContract()));
    mockCalendarClient.updateCalendarEvent.mockRejectedValue(
      new ServiceUnavailableException('Network'),
    );
  ...
  it('deletes external event then local Shift when linked', async () => {
    ...
    mockShiftModel.findOneAndDelete.mockReturnValue({
      exec: jest.fn().mockResolvedValue({}),
    });

    await service.remove(SHIFT_ID, BIZ_ID, {
      email: 'e',
      firstName: 'F',
      companyId: BIZ_ID,
    });
  ...
  it('treats provider 404 as idempotent success — local Shift deleted anyway', async () => {
    ...
    await expect(
      service.remove(SHIFT_ID, BIZ_ID, {
        email: 'e',
        firstName: 'F',
        companyId: BIZ_ID,
      }),
  ...
  it('preserves local Shift when external delete fails (non-404)', async () => {
    ...
    mockShiftModel.findOne.mockReturnValue(leanExec(linked));
    mockContractModel.findById.mockReturnValue(leanExec(makeContract()));
    mockCalendarClient.deleteCalendarEvent.mockRejectedValue(
      new ServiceUnavailableException('Provider error'),
    );
...
describe('RelayCalendarClient.deleteCalendarEvent — 404 handling', () => {
