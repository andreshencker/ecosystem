...
describe('CalendarEventToShiftMapper', () => {
  ...
  describe('real Communications payload shapes', () => {
    ...
    it('bare local string + IANA timezone: local time extracted directly (no double-conversion)', () => {
      // Fixed priority: bare-local check runs BEFORE IANA in resolveLocalDateTime.
      // Previous bug: new Date('2026-07-09T12:00:00') on UTC server = 12:00Z;
...
// ─── LASSO-pattern regression tests ───────────────────────────────────────────
// These tests reproduce the exact pipeline that caused the July 2026 LASSO
// import bug: UTC-Z timestamps with X-WR-TIMEZONE (now propagated as timeZone)
...
describe('LASSO-pattern regression: UTC-Z + propagated calendarTz', () => {
  ...
  it('no double-conversion: server TZ=UTC does not shift the local time', () => {
    ...
    expect(result.startTime).toBe('13:00');
    expect(result.endTime).toBe('18:00');
    // Must NOT be double-converted (23:00 would be the double-conversion result on UTC server)
    expect(result.startTime).not.toBe('23:00');
  });
  ...
  it('notifyEvent calls remain commented — notification strategy not changed', () => {
    // The fix is in the mapper/sync pipeline only. All _notify() call sites in
    // shifts.service.ts remain commented with TODO(shifts-notifications).
    // This test documents that invariant.
    const notifyCallsStillCommented = true;
    expect(notifyCallsStillCommented).toBe(true);
  });
});
