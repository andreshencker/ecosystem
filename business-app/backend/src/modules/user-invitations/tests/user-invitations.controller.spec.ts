...
describe('UserInvitationsController — POST /users/invite', () => {
  ...
  it('rejects platform_admin → business_owner (hierarchy check in controller)', async () => {
  ...
  it('rejects platform_admin → business_admin without targetCompanyId', async () => {
    const { controller } = await buildModule();
    await expect(
      controller.invite(fakeAuthContext(), {
        email: 'x@x.com',
  ...
  it('uses targetCompanyId for platform_admin → business_admin', async () => {
    const { controller, userInvitations } = await buildModule();
  ...
  it('ignores targetCompanyId for business_owner (uses actor companyId)', async () => {
    const actorDoc = fakeUserDoc({
      role: 'business_owner',
      scope: 'company',
      companyId: 'cmp_1',
