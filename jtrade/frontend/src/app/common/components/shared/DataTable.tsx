...
function MobileCard<T extends { id?: string }>({
    ...
    const primary =
        typeof config.primaryText === "function" ? config.primaryText(row) : String(row[config.primaryText] ?? "");
    ...
    return (
        <Card variant="outlined" sx={{ borderRadius: 2, cursor: onRowClick ? "pointer" : "default" }}>
                    ...
                    <>
                        ...
                        <Stack spacing={0.75} onClick={() => onRowClick?.(row)}>
                            {config.fields.map((f, i) => {
                                ...
                                const rendered = f.render ? f.render(rawValue, row) : String(rawValue ?? "—");
                                return (
                                    <Box key={i} display="flex" gap={1} alignItems="center">
                                        {f.label && (
                                            <Typography variant="caption" color="text.secondary" sx={{ width: "38%", flexShrink: 0 }}>
                                                {f.label}
                                            </Typography>
                                        )}
                                        <Box component="span" sx={{ flex: 1 }}>
                                            {typeof rendered === "string" ? (
                                                <Typography variant="body2" color="text.primary">{rendered}</Typography>
                                            ) : (
                                                rendered
                                            )}
                                        </Box>
            ...
            {actions && (
                <>
                    <Divider />
                    <CardActions sx={{ px: 2, py: 1, justifyContent: "flex-end" }}>{actions}</CardActions>
                </>
            )}
        </Card>
    );
...
export function DataTable<T extends { id?: string }>({
    ...
    const desktopColumns: GridColDef[] = rowActions
        ? [
            ...columns,
            {
                ...
                renderCell: (params) => (
                    <Box display="flex" alignItems="center" justifyContent="flex-end" height="100%" onClick={(e) => e.stopPropagation()}>
                ...
                {loading ? (
                    ...
                    emptyState ?? (
                        <Box py={4} textAlign="center">
                            <Typography variant="body2" color="text.secondary">{noRowsLabel}</Typography>
                        </Box>
                    )
                ) : (
                    <Stack spacing={1.5}>
                        {rows.map((row) => (
                            <MobileCard key={resolvedGetRowId(row)} row={row} config={mobileCardConfig!} onRowClick={onRowClick} rowActions={rowActions} />
                        ))}
                    </Stack>
                )}
    ...
    const grid = (
        <DataGrid
            ...
            sx={{
                ...
                "& .MuiDataGrid-columnHeaders": { backgroundColor: "background.default", borderBottom: "2px solid", borderColor: "divider" },
                "& .MuiDataGrid-columnHeaderTitle": { fontWeight: 600, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", color: "text.secondary" },
                "& .MuiDataGrid-row": { cursor: onRowClick ? "pointer" : "default", "&:hover": { backgroundColor: "action.hover" } },
                "& .MuiDataGrid-cell": { borderColor: "divider", display: "flex", alignItems: "center" },
                "& .MuiDataGrid-footerContainer": { borderTop: "1px solid", borderColor: "divider", backgroundColor: "background.default" },
                "& .MuiDataGrid-virtualScrollerContent": { minHeight: 240 },
            }}
            slots={{
                noRowsOverlay: () =>
                    emptyState ?? (
    ...
    return (
        <Box>
            {filterNode}
            {tableHeight ? <Box sx={{ height: tableHeight }}>{grid}</Box> : grid}
        </Box>
    );
}
