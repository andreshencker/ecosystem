...
export class CompanyService {
  ...
  // ==========================
  async findAll(params?: { active?: boolean }): Promise<CompanyResponseDto[]> {
    const filter: Record<string, any> = {};
    if (typeof params?.active === 'boolean') filter.isActive = params.active;

    const list = await this.model.find(filter).sort({ companyKey: 1 }).lean();
    return CompanyMapper.toResponseList(list as any[]);
  }
