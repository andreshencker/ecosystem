...
export class ProviderCredentialsService {
  ...
  async findAll(params: {
    ...
    // ✅ por defecto NO mandes encrypted al cliente
    q.select('-encrypted');
    ...
    return ProviderCredentialsMapper.toResponseList(list as any[]);
  }
  ...
  async getDecrypted(params: {
    ...
    const tag = this.normalizeTag(params.tag);
    if (!tag)
      throw new HttpException('tag is required', HttpStatus.BAD_REQUEST);

    const doc = await this.model
      .findOne({ companyChannelProviderId, tag, isActive: true })
      .lean();

    if (!doc)
      throw new HttpException('Credentials not found', HttpStatus.NOT_FOUND);

    return this.encryption.decryptJson((doc as any).encrypted);
  }
