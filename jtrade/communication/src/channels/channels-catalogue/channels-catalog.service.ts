...
export class ChannelsCatalogService {
  ...
  async findAll(params?: { active?: boolean }): Promise<ChannelResponseDto[]> {
    const filter: any = {};
    if (typeof params?.active === 'boolean') filter.isActive = params.active;

    const list = await this.model.find(filter).sort({ channelKey: 1 }).lean();
    return ChannelMapper.toResponseList(list as any[]);
  }
