...
This is the canonical source of truth for how Users, Companies, and Roles interact across the entire platform. All backend implementation and frontend RBAC work must align with this document. Previous decision DEC-003 documented the audit findings; this decision defines the final architecture.
...
**The platform layer manages identity. The communication engine manages the business domain.**

A Company in the communication engine is a tenant — a client of the platform operator. A User in the platform layer belongs to that tenant. The link between them is `User.companyId → Company._id`.
...
Represents a tenant (client organisation) using the communication platform.
...
Represents an authenticated human actor on the platform.
...
| Attribute | Value |
|---|---|
| **Purpose** | Operates the platform itself. Manages tenants (companies) and their access. |
...
**Ownership:** Owns the platform. Creates companies. Creates company_admin users (via invitation). Can deactivate/delete any user.
...
- Cannot access: platform settings, channels catalogue, provider master catalogue, other companies
...
This is a B2B communication platform. Companies are clients, not self-service consumers. Uncontrolled company creation creates operational risk (unmanaged tenants, orphaned data, billing complications). The Platform Admin controls which organisations are onboarded.
...
The invitation system is a new module: `src/platform/invitations/`.
  ...
  platform_admin: {
    ...
    sidebar: [
      ...
      { label: 'Settings', items: [
        ...
        { href: '/settings/platform', label: 'Platform Settings', icon: 'AdminPanelSettingsOutlined' },
      ]},
    ],
    allowedRoutes: ['*'],
...
| **Platform Settings** | Full | No access | No access | No access | `/settings/platform` |
