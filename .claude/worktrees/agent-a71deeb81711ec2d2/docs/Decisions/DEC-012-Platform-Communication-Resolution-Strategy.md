...
interface CommunicationContext {
  ownerCompanyId: ObjectId;          // always set — platform or tenant _id
...
```typescript
// For platform-owned communications
...
export class CommunicationContextResolver {
  ...
  // Build a platform-owned context
  async platformContext(
    channel: CommunicationChannel,
    type: CommunicationType
  ): Promise<CommunicationContext>
...
```typescript
{
  ownerCompanyId:    ObjectId,   // platform company or tenant company
