...
export type EventType = 'notification' | 'alert' | 'request' | 'security';
export type EventScope = 'platform' | 'company';
export type SenderScope = 'platform' | 'company';
...
export class EventCatalogue {
  ...
  /**
   * Determines which company's credentials are used to deliver this event.
   *
   *   'platform' — always route through the platform company (Grapifly) SMTP/API credentials.
   *               Required for all system events so new tenants receive email
   *               before they configure their own provider.
   *   'company'  — route through the target company's own credentials.
   *               Used for future tenant-owned events.
   */
  @Prop({
    required: false,
    enum: ['platform', 'company'],
