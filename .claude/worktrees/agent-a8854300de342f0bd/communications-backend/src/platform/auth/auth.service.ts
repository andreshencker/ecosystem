...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<{ message: string }> {
    ...
    // Fire-and-forget — routed through the platform company (DEC-009 Rev-2 §2).
   ...
   * Always routed through the platform company because new companies have no
  ...
  /**
   * Password reset email for platform administrators.
   * Always routed through the Platform Company (Grapifly) — same pattern as
  ...
  /**
   * Password-changed confirmation for platform administrators.
   * Always routed through the Platform Company (Grapifly) — same pattern as
   * notifyVerifyEmail(). originalCompanyId is logged for debugging only.
