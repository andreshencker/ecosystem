...
export class UsersController {
  ...
  async changePassword(
    ...
    if (before.scope === 'company' && before.companyId) {
      ...
      // Platform admin — route through platform company credentials.
      this.notificationService
        .notifyEvent({
          companyId: String(before.companyId),
  ...
  async deleteUser(
    ...
    if (actor.role === 'company_owner') {
      if (target.role === 'platform_admin') {
        throw new ForbiddenException('You cannot delete platform administrators');
      }
  ...
  async deactivateUser(
    ...
    if (actor.role === 'company_owner') {
      if (target.role === 'platform_admin') {
        throw new ForbiddenException('You cannot deactivate platform administrators');
      }
      if (target.companyId !== actor.companyId) {
  ...
  async reactivateUser(
    ...
    if (actor.role === 'company_owner') {
      if (target.role === 'platform_admin') {
        throw new ForbiddenException('You cannot reactivate platform administrators');
      }
      if (target.companyId !== actor.companyId) {
