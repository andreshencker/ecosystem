...
export class EventCatalogueService {
  ...
  async findByCompanyAndEventKey(params: {
    ...
    this.assertChannelContentMatchesDomain(
      doc.domainCatalogueId,
      doc.channelContent,
    );
  ...
  async create(
    ...
    const domain = await this.getDomainOrThrow(domainCatalogueId);
    ...
    this.assertChannelContentMatchesDomain(domain, channelContent);
  ...
  async update(
    ...
    if (dto.channelContent !== undefined) {
      const channelContent = dto.channelContent ?? {};
      this.assertChannelContentMatchesDomain(domain, channelContent);
      $set.channelContent = channelContent;
    }
  ...
  private assertChannelContentMatchesDomain(domain: any, channelContent: any) {
    const allowed = new Set(
      (domain?.channelsToUse ?? []).map((x: any) =>
        String(x.channel).toLowerCase().trim(),
      ),
    );

    const content = channelContent ?? {};
    for (const k of Object.keys(content)) {
      const ch = String(k).toLowerCase().trim();
      if (!allowed.has(ch)) {
        throw new HttpException(
          `channelContent includes "${ch}" but domain.channelsToUse does not allow it`,
          HttpStatus.BAD_REQUEST,
        );
      }
    }
