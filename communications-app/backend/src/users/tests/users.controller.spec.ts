...
async function buildModule(
  ...
  const usersServiceMock: Partial<Record<string, jest.Mock>> = {
    ...
    findById:             jest.fn(),
    listPlatformUsers:    jest.fn().mockResolvedValue({ items: [], total: 0, page: 1, limit: 25 }),
    listByCompanyId:      jest.fn().mockResolvedValue({ items: [], total: 0, page: 1, limit: 25 }),
    update:               jest.fn(),
    ...
    providers: [
      ...
      { provide: NotificationService, useValue: {
        notifyEvent: jest.fn().mockResolvedValue({
          ...
          companyId: 'plat_cmp',
          results: [{ channel: 'EMAIL', provider: 'gmail', success: true, error: null }],
        }),
      }},
      RolesGuard,
      Reflector,
  ...
  return {
    ...
    eventBus:     module.get(EventBusService) as typeof eventBusMock,
    notification: module.get(NotificationService) as any,
  };
}
...
describe('RolesGuard', () => {
  ...
  it('throws ForbiddenException when operator tries to access admin route', () => {
    jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(['platform_admin', 'company_owner', 'company_admin'] as any);
...
describe('UsersController — GET /users', () => {
  it('calls listPlatformUsers for platform_admin with no companyId filter', async () => {
    ...
    await controller.list(fakeAuthContext({ role: 'platform_admin', scope: 'global' }), '1', '25');
    expect(usersService.listPlatformUsers).toHaveBeenCalledWith(expect.objectContaining({ page: 1, limit: 25 }));
  });

  it('calls listByCompanyId when platform_admin provides companyId query param', async () => {
    const { controller, usersService } = await buildModule();
    await controller.list(fakeAuthContext({ role: 'platform_admin', scope: 'global' }), '1', '25', undefined, 'cmp_xyz');
    expect(usersService.listByCompanyId).toHaveBeenCalledWith('cmp_xyz', expect.objectContaining({ page: 1, limit: 25 }));
  });

  it('calls listByCompanyId scoped to actor companyId for company_admin', async () => {
    const actorDoc = fakeUserDoc({ role: 'company_admin', scope: 'company', companyId: 'cmp_1' });
    const { controller, usersService } = await buildModule({ findByIdOrThrow: jest.fn().mockResolvedValue(actorDoc) });
    await controller.list(fakeAuthContext({ role: 'company_admin', scope: 'company', companyId: 'cmp_1' }), '1', '25');
    expect(usersService.listByCompanyId).toHaveBeenCalledWith('cmp_1', expect.objectContaining({ page: 1, limit: 25 }));
  });

  it('parses page and limit as integers and clamps limit to 100', async () => {
    const { controller, usersService } = await buildModule();
    await controller.list(fakeAuthContext(), '2', '200');
    expect(usersService.listPlatformUsers).toHaveBeenCalledWith(expect.objectContaining({ page: 2, limit: 100 }));
  });
});
...
describe('UsersController — GET /users/me', () => {
  it('returns UserResponseDto for the current user', async () => {
    ...
    const { controller } = await buildModule({ findByIdOrThrow: jest.fn().mockResolvedValue(doc) });
...
describe('UsersController — PATCH /users/me', () => {
  it('forwards only allowed fields to UsersService.update', async () => {
    ...
    const { controller, usersService } = await buildModule({ update: jest.fn().mockResolvedValue(updated) });
    await controller.updateMe(fakeAuthContext(), { firstName: 'Updated', lastName: 'Name' });
    expect(usersService.update).toHaveBeenCalledWith('admin1', { firstName: 'Updated', lastName: 'Name' });
  });
});
...
describe('UsersController — PATCH /users/me/password', () => {
  it('emits USER_INVITATION_PASSWORD_COMPLETED event when wasMustChange was true', async () => {
    const actorDoc = fakeUserDoc({ mustChangePassword: true, scope: 'company', companyId: 'cmp_1' });
    const { controller, usersService, eventBus } = await buildModule({
      findByIdOrThrow: jest.fn().mockResolvedValue(actorDoc),
      changePassword:  jest.fn().mockResolvedValue({ ...actorDoc, mustChangePassword: false }),
    });
    await controller.changePassword(fakeAuthContext(), { currentPassword: 'old', newPassword: 'newPass!1' });
  ...
  it('does not emit event when mustChangePassword was false', async () => {
    const actorDoc = fakeUserDoc({ mustChangePassword: false, scope: 'company', companyId: 'cmp_1' });
    const { controller, usersService, eventBus } = await buildModule({
      findByIdOrThrow: jest.fn().mockResolvedValue(actorDoc),
