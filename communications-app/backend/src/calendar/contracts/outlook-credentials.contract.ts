...
export const OutlookCalendarCredentialsContract: ContractSpec<OutlookCalendarCredentials> =
  {
    ...
    normalize(input) {
      ...
      const clientId = strTrim(
        c.clientId ?? c.client_id ?? c.OUTLOOK_CLIENT_ID ?? c.AZURE_CLIENT_ID,
      );
      const clientSecret = strTrim(
        c.clientSecret ??
          c.client_secret ??
          c.OUTLOOK_CLIENT_SECRET ??
          c.AZURE_CLIENT_SECRET,
      );
      const refreshToken = strTrim(
        c.refreshToken ?? c.refresh_token ?? c.OUTLOOK_REFRESH_TOKEN,
      );
      const tenantId =
        strTrim(
          c.tenantId ?? c.tenant_id ?? c.OUTLOOK_TENANT_ID ?? c.AZURE_TENANT_ID,
        ) || 'common';
      const accessToken =
        strTrim(c.accessToken ?? c.access_token ?? c.OUTLOOK_ACCESS_TOKEN) ||
        undefined;
      const expiresAt = num(
        c.expiresAt ?? c.expires_at ?? c.OUTLOOK_TOKEN_EXPIRES_AT,
      );
      ...
      return {
        value: pick<OutlookCalendarCredentials>(
          normalized,
          ALLOWED,
        ) as OutlookCalendarCredentials,
      };
    },
    ...
    validate(value) {
      ...
      const isSpecialTenant =
        tid === 'common' || tid === 'consumers' || tid === 'organizations';
    ...
    async verify(value) {
      ...
      return {
        ok: true,
        message:
          'Outlook Calendar credentials format is valid. Live token exchange pending (Phase 2).',
      };
    },
  };
