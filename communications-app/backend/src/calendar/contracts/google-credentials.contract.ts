...
export const GoogleCalendarCredentialsContract: ContractSpec<GoogleCalendarCredentials> = {
  ...
  normalize(input) {
    ...
    const clientSecret = strTrim(c.clientSecret ?? c.client_secret ?? c.GOOGLE_CLIENT_SECRET);
    const refreshToken = strTrim(c.refreshToken ?? c.refresh_token ?? c.GOOGLE_REFRESH_TOKEN);
    const accessToken  = strTrim(c.accessToken  ?? c.access_token  ?? c.GOOGLE_ACCESS_TOKEN) || undefined;
    const expiresAt    = num(c.expiresAt ?? c.expires_at ?? c.GOOGLE_TOKEN_EXPIRES_AT);
    ...
    return { value: pick<GoogleCalendarCredentials>(normalized, ALLOWED) as GoogleCalendarCredentials };
  },
  ...
  validate(value) {
    ...
    if (value.clientId && !value.clientId.endsWith('.apps.googleusercontent.com')) {
  ...
  async verify(value) {
    ...
    return {
      ok: true,
      message: 'Google Calendar credentials format is valid. Live token exchange pending (Phase 2).',
    };
  },
};
