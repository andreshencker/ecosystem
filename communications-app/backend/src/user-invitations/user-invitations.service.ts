...
export class UserInvitationsService implements OnModuleInit {
  ...
  async sendInvitation(params: {
    ...
    const companyName = await this.resolveInvitationCompanyName(
      params.companyId,
    );
  ...
  async resendInvitation(
    ...
    const tempPassword = await this.usersService.refreshTemporaryPassword(
      invitation.userId,
    );
   ...
   */
  private async resolveInvitationCompanyName(
    companyId: string | null,
  ): Promise<string> {
  ...
  private async dispatchInviteNotification(params: {
    ...
    if (
      params.actorRole === 'platform_admin' &&
      params.targetRole === 'company_admin'
    ) {
  ...
  private async notifyPlatformAdminInvitation(params: {
    ...
    return this.firePlatformNotification(
      'security.platform_admin_invitation',
      params.email,
      {
        firstName: params.firstName,
        email: params.email,
  ...
  private async notifyCompanyWelcomeMessage(params: {
    ...
    await this.firePlatformNotification(
  ...
  private async firePlatformNotification(
    ...
    const platformCompanyId = await this.usersService
      .getPlatformCompanyId()
      .catch(() => null);
    if (!platformCompanyId) {
      this.logger.warn(
        `[${event}] platform company not found — notification skipped for ${email}`,
      );
      return false;
    }
    this.logger.log(
      `[${event}] platformCompanyId=${platformCompanyId} recipient=${email}`,
    );
    ...
    try {
      ...
      if (!delivered) {
        const errors = result.results
          ...
          .join('; ');
        this.logger.warn(
          `[${event}] notification did not deliver for ${email}: ${errors}`,
        );
      }
      return delivered;
    } catch (err: any) {
      this.logger.warn(
        `[${event}] notification threw for ${email}: ${err?.message}`,
      );
      return false;
    }
  }
