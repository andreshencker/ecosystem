...
describe('UserInvitationsController — invite hierarchy (DEC-009 §4 Rev-1)', () => {
  ...
  describe('company_owner actor', () => {
    ...
    it.each(['company_admin', 'operator', 'viewer'] as const)(
      'can invite %s — delegates with company_user_invitation scenario',
      async (role) => {
    ...
    it.each(['platform_admin', 'company_owner'])(
      'cannot invite %s → 403',
      async (role) => {
        await expect(
  ...
  describe('company_admin actor', () => {
    ...
    it.each(['platform_admin', 'company_owner', 'company_admin'])(
      'cannot invite %s → 403',
      async (role) => {
        await expect(
          controller.invite(adminCtx, {
