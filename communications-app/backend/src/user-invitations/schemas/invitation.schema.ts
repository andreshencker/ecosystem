 ...
 *              - platform_admin → company_owner via POST /companies/with-owner
 *              - company_owner/admin → company_admin/operator/viewer via POST /users/invite
 */
...
export class Invitation {
   ...
   * the invitation record is also created by POST /companies/with-owner for company_owner.
   * POST /users/invite enforces INVITE_HIERARCHY and never produces company_owner invitations.
   */
  @Prop({
    type: String,
    required: true,
   ...
   * 'modules' → issued by platform_admin for platform_admin or company_owner.
   * 'company'  → issued by company_owner/admin for internal company roles.
   */
  @Prop({
    type: String,
    required: true,
