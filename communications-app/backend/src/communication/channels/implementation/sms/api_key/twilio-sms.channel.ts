...
import { smsSendTimeout } from '../../shared/channel-timeout.util';
...
export class TwilioSmsChannel implements ISmsChannel {
  ...
  async sendSms(
    ...
    try {
      ...
      const sendOp = client.messages
        .create({ from: creds.fromNumber, to: payload.to, body: payload.text })
        .then(() => ({
      ...
      return await Promise.race([sendOp, smsSendTimeout(String(providerKey))]);
    } catch (err: any) {
