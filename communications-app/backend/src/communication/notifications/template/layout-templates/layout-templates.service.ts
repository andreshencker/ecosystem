...
export class LayoutTemplatesService {
  ...
  async findAllByCompanyFlat(params: {
    ...
    const themes = await this.themeModel
      .find({ companyId })
      .select('_id label isActive isDefault companyId')
      .lean();
  ...
  async create(
    ...
    this.assertHasContentPlaceholder(dto.html);
  ...
  // ─────────────────────────────
  // DEC-017 §4.3 — every layout must contain {{content}}
  private assertHasContentPlaceholder(html: string): void {
    if (!String(html ?? '').includes('{{content}}')) {
      throw new HttpException(
        'Layout template HTML must contain a {{content}} placeholder (DEC-017 §4.3)',
        HttpStatus.BAD_REQUEST,
      );
    }
  }
