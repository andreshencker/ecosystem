...
export class CompanyDeletionService {
  ...
  constructor(
    ...
    private readonly integrationModel: Model<CompanyIntegrationDocument>,
  ...
  private async deleteIntegrations(
    companyId: Types.ObjectId,
    session: ClientSession,
  ): Promise<number> {
    this.logger.log('Deleting company integrations…');
    const { deletedCount = 0 } = await this.integrationModel.deleteMany(
      { companyId },
      { session },
    );
    this.logger.log(`Deleted ${deletedCount} company integrations.`);
    return deletedCount;
  }
