...
export interface OrganizationProfile {
  organizationId: string; name: string; slug: string; entityType: 'company' | 'individual'; status: 'active' | 'suspended'; isPlatform: boolean; isDefault: boolean;
  legalName: string; tagline: string; timezone: string; officialEmail: string; supportEmail: string; supportPhone: string; supportPhoneCountryCode: string; supportPhoneNumber: string; supportHours: string;
...
const TAB_FIELDS: Record<Exclude<TabKey, 'platform'>, (keyof OrganizationProfile)[]> = {
  ...
  contact: ['officialEmail', 'supportEmail', 'supportPhoneCountryCode', 'supportPhoneNumber', 'supportHours'],
...
const COUNTRY_PREFIXES = [
  ['AU', '+61'], ['CO', '+57'], ['US / CA', '+1'], ['MX', '+52'], ['ES', '+34'], ['GB', '+44'], ['NZ', '+64'],
  ['BR', '+55'], ['AR', '+54'], ['CL', '+56'], ['PE', '+51'], ['EC', '+593'], ['VE', '+58'], ['UY', '+598'],
  ['FR', '+33'], ['DE', '+49'], ['IT', '+39'], ['PT', '+351'], ['NL', '+31'], ['IE', '+353'], ['CH', '+41'],
  ['IN', '+91'], ['SG', '+65'], ['AE', '+971'], ['JP', '+81'], ['KR', '+82'], ['CN', '+86'], ['ZA', '+27'],
] as const;
...
export function OrganizationProfileTabs({ organization, ownerEmail, canManage, apiUrl, onSaved }: { organization: OrganizationProfile; ownerEmail: string; canManage: boolean; apiUrl: string; onSaved: (organization: OrganizationProfile) => void }) {
  ...
  return <section className="organization-profile-card">
    ...
    <form onSubmit={save}>
      ...
      {activeTab === 'contact' && <div className="profile-fields two-columns">{field('officialEmail', 'Official email', { type: 'email' })}{field('supportEmail', 'Support email', { type: 'email' })}<label className="profile-field"><span>Support phone</span><div className="phone-input-group"><select aria-label="Country prefix" disabled={!canManage} value={form.supportPhoneCountryCode || '+61'} onChange={(event) => { setForm({ ...form, supportPhoneCountryCode: event.target.value }); setState('idle'); }}>{COUNTRY_PREFIXES.map(([country, prefix]) => <option key={`${country}-${prefix}`} value={prefix}>{country} {prefix}</option>)}</select><input aria-label="Phone number" type="tel" disabled={!canManage} value={form.supportPhoneNumber || ''} placeholder="412 345 678" onChange={(event) => { setForm({ ...form, supportPhoneNumber: event.target.value }); setState('idle'); }}/></div></label>{field('supportHours', 'Support hours', { placeholder: 'Monday–Friday, 9:00–17:00' })}</div>}
