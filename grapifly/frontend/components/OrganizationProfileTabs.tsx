...
export function OrganizationProfileTabs({ organization, ownerEmail, canManage, apiUrl, onSaved }: { organization: OrganizationProfile; ownerEmail: string; canManage: boolean; apiUrl: string; onSaved: (organization: OrganizationProfile) => void }) {
  ...
  return <section className="organization-profile-card">
    ...
    <form onSubmit={save}>
      ...
      {activeTab === 'contact' && <div className="profile-fields two-columns">{field('officialEmail', 'Official email', { type: 'email' })}{field('supportEmail', 'Support email', { type: 'email' })}{field('supportPhoneCountryCode', 'Country prefix', { type: 'tel', placeholder: '+61' })}{field('supportPhoneNumber', 'Phone number', { type: 'tel', placeholder: '412 345 678' })}{field('supportHours', 'Support hours', { placeholder: 'Monday–Friday, 9:00–17:00' })}</div>}
